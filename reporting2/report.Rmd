...
## GraphBLAS
df$Tool = gsub("^GB-Batch-x1$", "GraphBLAS Batch (1 thread)", df$Tool)
df$Tool = gsub("^GB-Batch-x8$", "GraphBLAS Batch (8 threads)", df$Tool)
df$Tool = gsub("^GB-Incr-x1$", "GraphBLAS Incremental (1 thread)", df$Tool)
df$Tool = gsub("^GB-Incr-x8$", "GraphBLAS Incremental (8 threads)", df$Tool)
...
ggplot(df.aggregated, aes(x=ChangeSet, y=MetricValue, fill=Tool)) +
    ...
    scale_shape_manual(values = seq(0,32)) +
