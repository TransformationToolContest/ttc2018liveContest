...
/**
 * Runs an instance of the TTC 2018 benchmark. Uses a naive implementation that
 * reindexes the file after every change sequence and reruns the query.
 */
public class Launcher {
	...
	protected void applyChanges(int iChangeSequence, final StandaloneHawk hawk)
		...
		final File fInitial = new File(changePath, "initial.xmi");
	...
	private void applyChanges(File fInitial, File fChanges) throws Exception {
		...
		for (Resource r : emfChanges.getResource().getResourceSet().getResources()) {
			if (r.getURI().path().endsWith("initial.xmi")) {
				r.save(null);
			}
		}
	...
	protected void modelLoading(final StandaloneHawk hawk) throws Exception, Throwable {
		hawk.requestFolderIndex(changePath);
		hawk.waitForSync();
	}
