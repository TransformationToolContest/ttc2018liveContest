...
fn main() {
    ...
    for (val, _weight) in top3_changes.clone() {
        ...
        println!("{:?};\"Q2\";{};{};0;\"Initial\";\"Elements\";{:?}", tool, change_set, run_index, three_score_str);    
    }
    ...
    for round in 1 .. (sequences + 1) {
        ...
        // Return the previous top 3 score if no changes are found.
        if top3_changes.len() == 0 {
            println!("{:?};\"Q2\";{};{};{};\"Update\";\"Elements\";{:?}", tool, change_set, run_index, round, three_score_str);    
        } else {
